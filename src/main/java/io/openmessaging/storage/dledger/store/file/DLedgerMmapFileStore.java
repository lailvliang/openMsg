...
public class DLedgerMmapFileStore extends DLedgerStore {
    ...
    private long ledgerBeginIndex = -1; //日志的起始索引，默认为 -1。
    private long ledgerEndIndex = -1;   //下一条日志下标，默认为 -1。
    private long committedIndex = -1;   //已提交的日志索引。
    private long committedPos = -1;
    private long ledgerEndTerm;  //当前最大的投票轮次。
    private DLedgerConfig dLedgerConfig;  //DLedger 的配置信息。
    private MemberState memberState;  //状态机。
    private MmapFileList dataFileList;  //日志文件(数据文件)的内存映射Queue。
    private MmapFileList indexFileList;  //索引文件的内存映射文件集合。（可对标 RocketMQ MappedFIleQueue )。
    private ThreadLocal<ByteBuffer> localEntryBuffer;   //本地线程变量，用来缓存索引ByteBuffer。
    private ThreadLocal<ByteBuffer> localIndexBuffer;   //本地线程变量，用来缓存数据索引ByteBuffer。
    private FlushDataService flushDataService;   //数据文件刷盘线程。
    private CleanSpaceService cleanSpaceService;    //清除过期日志文件线程
    private boolean isDiskFull = false;  //磁盘是否已满

    private long lastCheckPointTimeMs = System.currentTimeMillis();  //上一次检测点（时间戳）

    private AtomicBoolean hasLoaded = new AtomicBoolean(false);  //是否已经加载，主要用来避免重复加载(初始化)日志文件。
    private AtomicBoolean hasRecovered = new AtomicBoolean(false);  //是否已恢复
    ...
    public DLedgerEntry appendAsLeader(DLedgerEntry entry) {
        ...
        //从本地线程变量获取一个数据与索引 buffer。其中用于存储数据的 ByteBuffer，其容量固定为 4M ，索引的 ByteBuffer 为两个索引条目的长度，固定为64个字节
        ...
        //锁定状态机，并再一次检测节点的状态是否是 Leader 节点
        synchronized (memberState) {
            ...
            //计算新的消息的起始偏移量，关于 dataFileList 的 preAppend 后续详细介绍其实现，然后将该偏移量写入日志的 bytebuffer 中。
    ...
    public void updateCommittedIndex(long term, long newCommittedIndex) { //主节点当前的投票轮次。 需提交日志序号。
        if (newCommittedIndex == -1
            || ledgerEndIndex == -1
            || term < memberState.currTerm()
            || newCommittedIndex == this.committedIndex) { //如果待更新提交序号为 -1 或 投票轮次小于从节点的投票轮次或主节点投票轮次等于从节点的已提交序号，则直接忽略本次提交动作。
            return;
        }
        if (newCommittedIndex < this.committedIndex
            || newCommittedIndex < this.ledgerBeginIndex) {
            //如果需提交日志序号小于已提交日志序号 或 需提交序号小于当前节点的最小有效日志序号，则输出警告日志[MONITOR]，并忽略本次提交动作
        ...
        DLedgerEntry dLedgerEntry = get(newCommittedIndex);    //尝试根据待提交序号从从节点查找数据，如果数据不存在，则抛出 DISK_ERROR 错误。
        PreConditions.check(dLedgerEntry != null, DLedgerResponseCode.DISK_ERROR);
        this.committedIndex = newCommittedIndex;
        this.committedPos = dLedgerEntry.getPos() + dLedgerEntry.getSize();  //更新 commitedIndex、committedPos 两个指针，DledgerStore会定时将已提交指针刷入 checkpoint 文件，达到持久化 commitedIndex 指针的目的
    }
